...
const Calendar: React.FC = () => {
	...
	const handleEventClick = (clickInfo: EventClickArg) => {
		...
		setEventStartDate(
			event.start ? nicaDate(event.start).format("YYYY-MM-DD") : "",
		);
		setEventEndDate(
			event.end ? nicaDate(event.end).format("YYYY-MM-DD") : "",
		);
					...
					<div className="flex flex-col px-2 overflow-y-auto custom-scrollbar">
						<div>
							<h5 className="mb-2 font-semibold text-gray-800 modal-title text-theme-xl dark:text-white/90 lg:text-2xl">
								{selectedEvent ? (
									<FormattedMessage id="calendar.edit_event" />
								) : (
									<FormattedMessage id="calendar.add_event" />
								)}
							</h5>
						...
						<div className="flex items-center gap-3 mt-6 modal-footer sm:justify-end">
							...
							>
								{selectedEvent ? (
									<FormattedMessage id="calendar.update_changes" />
								) : (
									<FormattedMessage id="calendar.add_event" />
								)}
							</button>
