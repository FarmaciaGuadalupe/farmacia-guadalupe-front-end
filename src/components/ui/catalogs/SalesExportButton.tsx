...
const salesColumnsConfig: ExcelColumnConfig<any>[] = [
  ...
  { 
    header: 'Empleado', 
    formatter: (sale) => sale.employee ? `${sale.employee.names || ''} ${sale.employee.lastnames || ''}`.trim() : 'N/A' 
  },
  { 
    header: 'Total Neto', 
    key: 'netTotal' 
  ...
  { 
    header: 'Productos', 
    formatter: (sale) => sale.saleDetails
  ...
  { 
    ...
    formatter: (sale) => sale.salePayments
  ...
  const handleExport = async () => {
    try {
      ...
      if (allNodes.length > 0) {
        ...
        toast.success('El reporte de ventas ha sido exportado con éxito.');
      } else {
