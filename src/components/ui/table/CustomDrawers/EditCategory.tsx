...
export default function EditCategory({ row, onClose }: EditCategoryProps) {
	const intl = useIntl();
	...
	const [isActive, setIsActive] = useState(categoryData.is_active ?? true);
	...
	const handleSubmit = async () => {
		...
		try {
			await updateCategoryMutation({
				variables: {
					id: parseInt(String(categoryData.category_id), 10),
					name: name.trim(),
			...
			toast.success(intl.formatMessage({ id: "category.update.success" }));
			onClose();
		} catch (err) {
			console.error("Error al actualizar la categoría:", err);
			toast.error(intl.formatMessage({ id: "category.update.error" }));
		}
	};
