...
import EditCategory from "../CustomDrawers/EditCategory";
...
const EditCategoryDrawer = ({ row, onClose }: EditCategoryDrawerProps) => {
	const intl = useIntl();
	...
	return (
		<CellWithDrawer
			...
			widthClass="w-150"
		>
			<EditCategory row={row} onClose={onClose} />
		</CellWithDrawer>
	);
};
...
const ToggleActivatedModal = ({ row, onClose }: ToggleActivatedModalProps) => {
	const intl = useIntl();
	const { category_id, is_active, name, description } = row.original;

	const [updateStatus, { loading }] = useMutation(UPDATE_CATEGORY_MUTATION, {
	...
	const handleConfirm = () => {
		updateStatus({
			variables: {
				id: parseInt(String(category_id), 10),
				name: name || "",
				description: description || "",
				is_active: !is_active,
			},
...
export const CategoryCellActions = ({ row }: CategoryCellActionsProps) => {
	const { original } = row;
