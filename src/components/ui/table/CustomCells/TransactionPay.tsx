...
const TransactionPay = ({ row }: any) => {
    ...
    const getPaymentIcon = (id: number, className = "size-4") => {
        switch (id) {
            case 1:
                return <BanknotesIcon className={`${className} text-green-600`} />;
            case 2:
                return <CreditCardIcon className={`${className} text-blue-600`} />;
            case 3:
                return <CalendarIcon className={`${className} text-purple-600`} />;
            default:
                return <CreditCardIcon className={`${className} text-gray-500`} />;
        }
    };
        ...
        <div className="flex flex-col gap-2 p-1">
            ...
            {payments.map((p: any, index: number) => (
                <div key={index} className="flex items-center gap-2">
                    {getPaymentIcon(p.paymentMethod?.paymentMethodId, "size-3.5")}
    ...
    return (
        <div className='flex flex-col justify-center h-full py-1'>
            ...
            >
                <div className={`flex flex-row items-center gap-2 ${hasMultiplePayments ? 'cursor-help' : ''}`}>
                    {hasMultiplePayments ? (
                        <CurrencyDollarIcon className="size-4 text-amber-500" />
                    ) : (
                        getPaymentIcon(payments[0]?.paymentMethod?.paymentMethodId)
                    )}
