...
import EditActiveIngredient from "../CustomDrawers/EditActiveIngredient";

const UPDATE_ACTIVE_INGREDIENT_MUTATION = gql`
...
interface EditActiveIngredientDrawerProps {
	row: {
...
interface MenuItem {
	...
	drawer: React.ComponentType<{
		row: { original: ActiveIngredientData };
		onClose: () => void;
	}>;
}

interface ActiveIngredientCellActionsProps {
	row: {
		original: ActiveIngredientData;
	};
}

const EditActiveIngredientDrawer = ({
	row,
	onClose,
}: EditActiveIngredientDrawerProps) => {
	const intl = useIntl();
	...
	return (
		<CellWithDrawer
			isOpen={true}
			onClose={onClose}
			title={intl.formatMessage({ id: "active_ingredient.edit" })}
			widthClass="w-150"
		>
			<EditActiveIngredient row={row} onClose={onClose} />
		</CellWithDrawer>
	);
};
...
const ToggleActivatedModal = ({ row, onClose }: ToggleActivatedModalProps) => {
	const intl = useIntl();
	const { active_ingredient_id, is_active, name, is_controlled } =
		row.original;

	const [updateStatus, { loading }] = useMutation(
		UPDATE_ACTIVE_INGREDIENT_MUTATION,
		{
	...
	const handleConfirm = () => {
		updateStatus({
			variables: {
				id: parseInt(String(active_ingredient_id), 10),
				name: name || "",
...
export const ActiveIngredientCellActions = ({
	row,
}: ActiveIngredientCellActionsProps) => {
	const { original } = row;
	...
	const items = [
		{
			...
			drawer: EditActiveIngredientDrawer,
		},
		{
			showWhen: true,
