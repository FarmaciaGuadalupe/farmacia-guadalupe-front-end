...
import EditSupplier from "../CustomDrawers/EditSupplier";

const UPDATE_SUPPLIER_STATUS_MUTATION = gql`
	mutation ToggleSupplierStatus($id: Int!, $isActive: Boolean) {
		updateSupplier(id: $id, isActive: $isActive) {
			supplier_id
			is_active
		}
	}
...
interface MenuItem {
	...
	drawer: React.ComponentType<{
		row: { original: SupplierData };
		onClose: () => void;
	}>;
}

interface SupplierCellActionsProps {
	row: {
		original: SupplierData;
	};
}

const EditSupplierDrawer = ({ row, onClose }: EditSupplierDrawerProps) => {
	const intl = useIntl();
	...
	return (
		<CellWithDrawer
			...
			widthClass="w-150"
		>
			<EditSupplier row={row} onClose={onClose} />
		</CellWithDrawer>
	);
};
...
const ToggleActivatedModal = ({ row, onClose }: ToggleActivatedModalProps) => {
	const intl = useIntl();
	const { supplier_id, is_active, company_name } = row.original;

	const [updateStatus, { loading }] = useMutation(
		UPDATE_SUPPLIER_STATUS_MUTATION,
		{
	...
	const handleConfirm = () => {
		updateStatus({
			variables: {
				id: parseInt(String(supplier_id), 10),
				isActive: !is_active,
			},
		});
	};
...
export const SupplierCellActions = ({ row }: SupplierCellActionsProps) => {
	const { original } = row;
