...
const ToggleActivatedModal = ({ row, onClose }: any) => {
    ...
    return (
        <SimpleModal
            isOpen={true}
            onClose={onClose}
            title={intl.formatMessage({ id: is_active ? 'deactivate' : 'activate' }) + ` ${name}`}
        >
            ...
            <div className="mt-6 flex justify-end gap-3">
                ...
                <button 
                    ...
                    className={clsx(
                        ...
                        is_active ? "bg-red-600 hover:bg-red-700" : "bg-green-600 hover:bg-green-700",
                ...
                >
                    {loading ? (
                        ...
                        is_active ? <FormattedMessage id="deactivate" /> : <FormattedMessage id="activate" />
                    )}
                </button>
            </div>
        </SimpleModal>
...
export const BrandCellActions = ({ row }: any) => {
    ...
    console.log('Row in BrandCellActions:', row);
    console.log('Original in BrandCellActions:', original);
    console.log('is_active in BrandCellActions:', is_active);

    const items = [
        {
            showWhen: true,
            label: <Fragment><PencilSquareIcon className='size-4.5 stroke-1' /><span><FormattedMessage id='edit' /></span></Fragment>,
